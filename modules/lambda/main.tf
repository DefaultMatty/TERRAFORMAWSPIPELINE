...
data "archive_file" "zipit" {
  type = "zip"
  source_file = "lambda_scripts/lambda_${var.function}.py"
  output_path = "lambda_${var.function}.zip"
}

resource "aws_lambda_function" "lambda_function" {
  depends_on = [data.archive_file.zipit]
  role = var.lambda_role_arn
  filename = "lambda_${var.function}.zip"
  function_name = var.function
  handler = "lambda_${var.function}.lambda_handler"
  layers = ["arn:aws:lambda:eu-west-2:336392948345:layer:AWSSDKPandas-Python38:5"]
  runtime = "python3.8"
  timeout = 900 
  memory_size = 3008
  source_code_hash = filebase64sha256("lambda_${var.function}.zip")
}
